...
describe( 'Using Plugins API', () => {
	...
	describe( 'Annotations', () => {
		...
		// It should trigger the counter two times:
		// 1. From `render` because the props changed (annotations prop).
		// 2. From `componentDidUpdate` because that has logic to detect format
		//    props changing and applying those to the actual DOM.
		const ANNOTATION_RERENDERS = 2;
		...
		it( 'Should re-render thrice when removing an annotation', async () => {
			await page.keyboard.type( 'Title' + '\n' + 'RerenderCounter' + '\n' );
			await annotateFirstBlock( 0, 10 );
